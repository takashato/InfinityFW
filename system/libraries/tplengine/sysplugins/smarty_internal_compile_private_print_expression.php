<?php
/**
 * tplengine Internal Plugin Compile Print Expression
...
/**
 * tplengine Internal Plugin Compile Print Expression Class
 *
 * @package    tplengine
 * @subpackage Compiler
 */
                ...
                if (!empty($compiler->template->smarty->registered_filters[ Smarty::FILTER_VARIABLE ])) {
                    foreach ($compiler->template->smarty->registered_filters[ Smarty::FILTER_VARIABLE ] as $key =>
                        ...
                        if (!is_array($function)) {
                            ...
                            $output =
                                "\$_smarty_tpl->smarty->registered_filters[tplengine::FILTER_VARIABLE]['{$key}'][0]->{$function[1]}({$output},\$_smarty_tpl)";
                        } else {
    ...
    private function compile_output_filter(Smarty_Internal_TemplateCompilerBase $compiler, $name, $output)
    {
        ...
        if ($path) {
            ...
            if ($compiler->template->caching) {
                $compiler->parent_compiler->template->compiled->required_plugins[ 'nocache' ][ $name ][ tplengine::FILTER_VARIABLE ][ 'file' ] =
                    $path;
                $compiler->parent_compiler->template->compiled->required_plugins[ 'nocache' ][ $name ][ tplengine::FILTER_VARIABLE ][ 'function' ] =
                    $plugin_name;
            } else {
                $compiler->parent_compiler->template->compiled->required_plugins[ 'compiled' ][ $name ][ tplengine::FILTER_VARIABLE ][ 'file' ] =
                    $path;
                $compiler->parent_compiler->template->compiled->required_plugins[ 'compiled' ][ $name ][ tplengine::FILTER_VARIABLE ][ 'function' ] =
                    $plugin_name;
            }
             * */
